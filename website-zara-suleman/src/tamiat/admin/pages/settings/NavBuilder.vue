...
export default {
	...
	methods: {
		...
		moveLinkUp(item, previousItem) {
			...
			const previousItemCopy = Object.assign({}, previousItem);
		...
		moveLinkDown(item, nextItem) {
			const itemCopy = Object.assign({}, item);
			const nextItemCopy = Object.assign({}, nextItem);
		...
		moveSubLinkUp(currentKey, parent) {
			...
			const childrenArray = this.convertFirebaseObjToArray(parent.children);
			let previousKey = '';
