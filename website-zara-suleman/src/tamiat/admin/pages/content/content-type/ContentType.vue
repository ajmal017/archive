...
import { contentsRef, fieldsRef } from '../../../firebase_config';
...
export default {
	...
	computed: {
		checkedFields() {
			return this.fields.filter(field => field.checked);
		}
	},
	methods: {
		loadContentTypes() {
		...
		createMenuItem(edit) {
			...
			const selectedFields = this.checkedFields.map(field => ({
				name: field.name,
				type: field.type,
		...
		moveFieldUp(field, previousField) {
			const itemCopy = Object.assign({}, field);
			const previousItemCopy = Object.assign({}, previousField);
		...
		moveFieldDown(field, nextField) {
			const itemCopy = Object.assign({}, field);
			const nextItemCopy = Object.assign({}, nextField);
		...
		mapFields() {
			for (let j = 0; j < this.fields.length; j++) {
				for (let i = 0; i < this.selectedContent.fields.length; i++) {
